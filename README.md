...
- Refreshes every 5 minutes, after wake, and as soon as the network comes back (at login it usually starts before Wi-Fi is up). Other failures retry quickly (15s, 30s, … up to 5 min). The usage API rate-limits harder than that, so on HTTP 429 it backs off up to 30 minutes. The item dims when the last successful refresh is over 15 minutes old, and the right-click menu shows the error and last update time.
